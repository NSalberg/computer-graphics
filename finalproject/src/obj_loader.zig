...
const zlm = @import("zlm"); // Assuming this is your math lib
const Vec3 = zlm.Vec3;
const Vec2 = zlm.Vec2;

// Must match your existing Vertex definition exactly
const Vertex = struct {
    pos: Vec3,
    normal: Vec3,
    uv: Vec2,
...
pub fn parseObj(allocator: std.mem.Allocator, file_content: []const u8) ![]Vertex {
    var temp_positions = std.ArrayList(Vec3).init(allocator);
    defer temp_positions.deinit();

    var temp_normals = std.ArrayList(Vec3).init(allocator);
    defer temp_normals.deinit();

    var temp_uvs = std.ArrayList(Vec2).init(allocator);
    defer temp_uvs.deinit();

    var final_vertices = std.ArrayList(Vertex).init(allocator);
    ...
    while (lines.next()) |line| {
        ...
        if (std.mem.eql(u8, type_str, "v")) {
            ...
            try temp_positions.append(Vec3.new(x, y, z));
        } else if (std.mem.eql(u8, type_str, "vn")) {
            // -- Vertex Normal --
            const x = try parseFloat(tokens.next());
            const y = try parseFloat(tokens.next());
            const z = try parseFloat(tokens.next());
            try temp_normals.append(Vec3.new(x, y, z));
        } else if (std.mem.eql(u8, type_str, "vt")) {
            // -- Vertex UV --
            const u = try parseFloat(tokens.next());
            const v = try parseFloat(tokens.next());
            // OBJ UVs often have V flipped relative to OpenGL.
            // If textures look upside down, change to (1.0 - v)
            try temp_uvs.append(Vec2.new(u, v));
            ...
            while (tokens.next()) |face_token| {
                if (count >= 4) break; // We only support quads max for simplicity
            ...
            // Triangle 1: 0, 1, 2
            if (count >= 3) {
                try final_vertices.append(face_verts[0]);
                try final_vertices.append(face_verts[1]);
                try final_vertices.append(face_verts[2]);
            }
            // Triangle 2: 0, 2, 3 (if it was a quad)
            if (count == 4) {
                try final_vertices.append(face_verts[0]);
                try final_vertices.append(face_verts[2]);
                try final_vertices.append(face_verts[3]);
            }
        }
    }
