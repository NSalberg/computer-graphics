...
public:
  ...
  //  or as just a list of raw bytes (for loading from a file, or cute tricks
  //  like quickly setting everything to 0)
  union PixelData {
    Pixel *pixels;
    uint8_t *raw;
  };
...
public:
  ...
  Pixel &GetPixel(int x, int y) const {
    if (ValidCoord(x, y)) {
      ...
      throw std::out_of_range("GetPixel: coordinates (" + std::to_string(x) +
                              ", " + std::to_string(y) +
                              ") are out of bounds (" + std::to_string(width) +
