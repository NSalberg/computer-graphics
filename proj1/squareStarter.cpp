...
void mouseClicked(float m_x, float m_y){
   ...
   //
   // do_rotate = if (pointPolyCornerDist(clicked_mouse, poly) ){
   //
   // };
   do_rotate, do_scale, do_translate = false;
   //oops this should scale the rect
   do_rotate = pointPolyCornerDist(clicked_mouse, poly) < 0.01;
   printf("corner dist: %f %d\n", pointPolyCornerDist(clicked_mouse, poly), do_rotate);
   do_scale = false;

   if (!do_rotate and !do_scale) {
      do_translate = pointInPoly(clicked_mouse, poly);
   }
}
...
void mouseDragged(float m_x, float m_y){
   ...
   if (do_rotate){
      ...
      printf("angle %f, sin %d, rec_pos %f %f\n", a, sin, rect_pos.x, rect_pos.y);
      rect_angle = clicked_angle + a * sin; // FIX ME - Replace this line with your code to compute the new angle
   }
...
int main(int argc, char *argv[]){
   ...
   SDL_Window* window = SDL_CreateWindow("Project 1", screen_width, screen_height, SDL_WINDOW_OPENGL);
   if (!window) {
   ...
   glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE); // GL_CLAMP_TO_EDGE, GL_MIRRORED_REPEAT, GL_REPEAT, etc.
   glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
   glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); // GL_NEAREST, GL_LINEAR, etc.
   glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR); // GL_NEAREST, GL_LINEAR, etc.
